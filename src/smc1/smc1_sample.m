...
function state = smc1_sample(state, data, params, t, s, delstep)
...
temp = ind_t;
temp_t = t;
while  size(temp, 1) == 0  &&  temp_t > 0
	disp('ZERO OBSERVATIONS FOR t = '); disp(temp_t);
	temp_t = t-1;
...
if temp_t > 0
	...
	final_obs = 0;    % will this work with the code below?..i call state{3}(final_obs, k)...
end
...
% update cluster params (based on assignments &/or transition kernel. for s=1, t>1, initialize to clusters at previous time)

for k = 1 : K

	if  t > 1  &&  s == 1    % if s is first sample of a time-step > 1
	
		state{2}{t,k} = state{2}{t-1,k};    % initialize cluster params to previous cluster params

	else

		if state{3}(final_obs, k) > 0;    % if cluster is alive

			if t == 1    % if t = 1 (and for any s)

				data_k = data_t(find(state{1}(ind_t_range) == k), :);    % get all obs (of t=1) assigned to cluster k

				params_post = update_mvnmn_params2(data_k(:, 1:2), data_k(:, 3:end-1), params);

			else    % if s > 1  &&  t > 1

				data_k = data_t(find(state{1}(ind_t_range) == k), :);    % get all obs (of current t) assigned to cluster k

				if size(data_k, 1) > 0  &&  length(state{2}{t-1,k}) > 0     % if obs assigned and there exists prev cluster params
				...
				elseif length(state{2}{t-1,k}) > 0    % if no obs assigned (and there exists prev cluster params)

					aux_mvn = mvnrnd(state{2}{t-1,k}{1}, state{2}{t-1,k}{2}, params{3});
					aux_mn = mnrnd(params{11}, state{2}{t-1,k}{3}, params{3}+50);
					params_post = update_mvnmn_params2(aux_mvn, aux_mn, params);

				else    % if there does not exist previous cluster params (and there must be obs assigned since cluster is alive)
...
% first, clear all sizes past ind_t(1)

%state{3}(ind_t(1):end, :) = repmat(state{3}(ind_t(1), :), [size(state{3}(ind_t(1):end, :), 1), 1]);
...
for i = 1 : size(data_t,1)
	...
	prob_obs(K+1) = my_log_niwdir_coefratio(x, params);  % "predictive marginal"

	% prob_vec is the discrete probability vector from which assignment is sampled
