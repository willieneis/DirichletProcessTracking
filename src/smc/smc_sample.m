% NOTE: I never ended up using this function. Instead, I always used smc1_sample.m.
% ---------------------------------------------------------------------------------
function particles = smc_sample(particles,data,params,t,s,numsamples)
% For each particle, take a single gibb sample (sample each alive cluster's
%   parameters, each assignment, possibly perform a deletion step and update
%   the size matrix). Return an updated set of particles.
    
    addpath('../my_log_prob/','../prob/','../mcmc/');

    for pInd = 2:length(particles)
        ...
        for i = 1:size(data_t,1)
            ...
            if sample_k == K+1
                K = K+1;
                state = smc_newclust(state,params,x,t);
